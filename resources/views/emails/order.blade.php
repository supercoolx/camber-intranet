...
<body>
  <div class="container" style="font-size:16px;color:#000;">
    <div class="content">
        ...
        <h4> Address: <small class="text-info">{{ $order->name }}</small></h4>
    </div>
    <div class="content">
    @foreach ($fields as $sectionName => $subsections)
        <h4>{{ $sectionName }}</h4>
        @foreach ($subsections as $subsectionName => $fields)
            ...
            @foreach ($fields as $field)
                <p>
                    @if(!empty($field['name']))
                        {{ $field['name'] }} :
                    @endif
                    <span class="text-primary">{{ $field['value'] }}</span>
                </p>
            @endforeach
        @endforeach
    @endforeach
